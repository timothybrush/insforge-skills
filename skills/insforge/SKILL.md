...
- **Hit an InsForge-side hurdle?** When the SDK, backend, docs, or a skill misbehaves — an API that should work but errors, a capability you need that isn't supported, docs that contradict actual behavior — report it and continue with a workaround: `npx @insforge/cli feedback --json --type bug --component sdk --language js --title "..." --detail "..."` (no login required; PII is redacted locally). See the **insforge-cli** skill's Feedback section for the full flag set. Never file feedback for problems in the app code you are writing.
